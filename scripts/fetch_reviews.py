...
from bk_maps.config import LOG_DIR, LOG_FILE
...
async def main():
    ...
    try:
        ...
        logger.info(f"Found {len(place_ids)} locations to process")
        
        # Process places and get reviews
        reviews = places_client.get_places_details_and_reviews(place_ids)
        logger.info(f"Successfully fetched reviews for {len(reviews)} locations")
        
        # Save to BigQuery
        bigquery_client.save_reviews(reviews)
